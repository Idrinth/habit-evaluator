export type Language = 'en' | 'de' | 'es' | 'fr';

export const LANGUAGES: { code: Language; label: string }[] = [
	{ code: 'en', label: 'English' },
	{ code: 'de', label: 'Deutsch' },
	{ code: 'es', label: 'Español' },
	{ code: 'fr', label: 'Français' }
];

const translations: Record<Language, Record<string, string>> = {
	en: {
		'nav.home': 'Home',
		'nav.edit': 'Edit',
		'nav.addHabit': 'Add Habit',
		'nav.addCategory': 'Add Category',
		'nav.manageCategories': 'Categories',
		'nav.scoringRules': 'Scoring Rules',
		'nav.points': 'Point Charts',
		'nav.diary': 'Diary',
		'nav.sleep': 'Sleep',
		'nav.stats': 'Statistics',
		'nav.emotions': 'Emotions',
		'nav.exportPdf': 'Export PDF',
		'nav.backup': 'Backup',
		'nav.medication': 'Medication',
		'nav.translations': 'Translations',
		'nav.logout': 'Logout',
		'emotions.graphTitle': 'Emotion Pairs Graph',
		'emotions.week': 'Week',
		'emotions.month': 'Month',
		'emotions.loading': 'Loading emotion data...',
		'emotions.noPairs': 'No emotion pairs found. Create some emotion pairs first.',
		'emotions.avg': 'Avg',
		'emotions.entries': 'Entries',
		'emotions.positive': 'Positive',
		'emotions.negative': 'Negative',
		'emotions.avgLine': 'Average',
		'home.title': 'Your Habits',
		'home.loading': 'Loading habits...',
		'home.noHabits': 'No habits yet.',
		'home.loadDefaults': 'Load Default Habits',
		'home.loadingDefaults': 'Loading defaults...',
		'home.defaultsLoaded': 'Default habits loaded successfully',
		'home.allCategories': 'All categories',
		'home.complete': 'Complete',
		'home.undo': 'Undo',
		'home.delete': 'Delete',
		'home.confirmDelete': 'Are you sure you want to delete this habit?',
		'home.todayEntries': 'today',
		'home.dailyLimit': 'Daily limit reached',
		'eval.title': 'Evaluation',
		'eval.streak': 'Current streak',
		'eval.longestStreak': 'Longest streak',
		'eval.completionRate': 'Completion rate',
		'eval.completions': 'Completions',
		'eval.target': 'Target',
		'eval.days': 'days',
		'eval.onTrack': 'On track',
		'eval.behind': 'Behind',
		'eval.weeklyScore': 'Weekly Score',
		'eval.currentScore': 'Current',
		'eval.predictedScore': 'Predicted',
		'eval.points': 'pts',
		'eval.avoidanceRate': 'Avoidance rate',
		'eval.avoidanceStreak': 'Avoidance streak',
		'firstStart.title': 'Welcome to Habit Evaluator',
		'firstStart.notProfessionalHelp': 'This app is a self-tracking tool and does not replace professional medical or psychological help. If you are struggling, please reach out to a qualified professional.',
		'firstStart.noDataSharing': 'By default, all your data stays on your device and is not shared with anyone. You can optionally configure remote storage in the settings.',
		'firstStart.acknowledge': 'I understand',
		'nav.imprint': 'Project legal',
		'imprint.title': 'Project legal',
		'imprint.contact': 'Contact',
		'imprint.name': 'Name',
		'imprint.email': 'Email',
		'imprint.license': 'License',
		'imprint.licenseText': 'This project is licensed under the MIT License.',
		'imprint.dataProtection': 'Data Protection',
		'imprint.noRemoteStorage': 'No remote storing of data unless explicitly enabled by the user.',
		'imprint.noTracking': 'No tracking.',
		'imprint.noDataUsage': 'No usage of any provided data besides providing the service.',
		'imprint.disclaimers': 'Disclaimers',
		'imprint.notProfessionalHelp': 'This app is a self-tracking tool and does not replace professional medical or psychological help. If you are struggling, please reach out to a qualified professional.',
		'imprint.noDataSharing': 'By default, all your data stays on your device and is not shared with anyone. You can optionally configure remote storage in the settings.',
		'imprint.variantDisclaimer': 'The authors of this project are not responsible for modifications made by third-party variants or the actions of webservice hosters.',
		'imprint.libraries': 'Third-Party Libraries',
		'imprint.libraryName': 'Library',
		'imprint.libraryVersion': 'Version',
		'imprint.libraryLicense': 'License',
		'correlations.title': 'Event Correlations',
		'correlations.description': 'Time-weighted Pearson correlations across habits, diary, sleep, and emotions over the past year. Newer data is weighted higher.',
		'correlations.eventA': 'Event A',
		'correlations.eventB': 'Event B',
		'correlations.correlation': 'Correlation',
		'correlations.sharedDays': 'Shared Days',
		'correlations.confidence': 'Confidence',
		'correlations.strong': 'Strong',
		'correlations.moderate': 'Moderate',
		'correlations.weak': 'Weak',
		'correlations.loading': 'Loading correlations...',
		'correlations.noData': 'No correlations found. You need at least 7 days of overlapping data between two tracked events.',
		'correlations.disclaimer': 'Correlation does not imply causation. Weak correlations (|r| < 0.3) have low statistical confidence and should be interpreted with caution.',
		'correlations.backToStats': 'Back to Statistics',
		'nav.correlations': 'Correlations',
		'nav.settings': 'Settings',
		'nav.planner': 'Planner',
		'nav.foodLog': 'Food Log',
		'nav.sportGraph': 'Sport Graph',
		'nav.foodDistribution': 'Food Graph',
		'foodDistribution.title': 'Daily Food Distribution',
		'foodDistribution.description': 'Meal frequency and nutritional averages by hour of day over the last 30 days.',
		'foodDistribution.loading': 'Loading food distribution data...',
		'foodDistribution.noData': 'No food log entries in the last 30 days.',
		'foodDistribution.mealsPerHour': 'Meals per Hour',
		'foodDistribution.avgKcalPerHour': 'Average Kcal per Hour',
		'foodDistribution.avgCarbsPerHour': 'Average Carbs (g) per Hour',
		'foodDistribution.meals': 'meals',
		'foodDistribution.totalMeals': 'Total meals (30 days)',
		'foodDistribution.backToFoodLog': 'Back to Food Log',
		'settings.moduleVisibility': 'Module Visibility',
		'settings.moduleVisibilityHint': "Hide modules you don't use to simplify navigation.",
		'settings.moduleDiary': 'Diary',
		'settings.moduleSleep': 'Sleep Tracking',
		'settings.moduleEmotions': 'Emotions',
		'settings.modulePoints': 'Point Charts',
		'settings.moduleStatistics': 'Statistics',
		'settings.moduleFoodLog': 'Food Log',
		'settings.moduleSportLog': 'Sport Log',
		'settings.moduleMedication': 'Medication',
		'settings.moduleBackup': 'Backup',
		'settings.modulePdfExport': 'PDF Export',
		'settings.moduleDayPlanner': 'Planner',
		'planner.weekPlanner': 'Week Planner',
		'planner.weekPlannerDescription': 'Plan your week by assigning activity groups to time slots.',
		'planner.weekOverview': 'Week Overview',
		'planner.weekOverviewDescription': 'Visual overview of occupied time slots across the week.',
		'planner.weekOverviewLoading': 'Loading week overview...',
		'planner.weekOverviewEmpty': 'No time slots assigned yet. Use the Week Planner to assign activity groups to time slots.',
		'planner.weekOverviewBackToPlanner': 'Back to Planner',
		'planner.weekOverviewOccupied': 'occupied',
		'planner.weekOverviewFree': 'free',
		'planner.activities': 'Activities',
		'planner.activitiesDescription': 'Manage activities and assign them to groups.',
		'planner.groups': 'Activity Groups',
		'planner.groupsDescription': 'Create and manage groups to organize your activities.',
		'planner.groupsLoading': 'Loading groups...',
		'planner.noGroups': 'No groups yet. Create a group to get started.',
		'planner.noGroupActivities': 'No activities in this group',
		'planner.groupName': 'Name',
		'planner.groupDescription': 'Description (optional)',
		'planner.addGroup': 'Add Group',
		'planner.editGroup': 'Edit Group',
		'planner.saveGroup': 'Save',
		'planner.deleteGroup': 'Delete',
		'planner.cancelGroup': 'Cancel',
		'planner.groupNameRequired': 'Please enter a group name.',
		'planner.groupBackToPlanner': 'Back to Planner',
		'planner.activitiesLoading': 'Loading activities...',
		'planner.noActivities': 'No activities yet. Create an activity to get started.',
		'planner.activityName': 'Name',
		'planner.activityDescription': 'Description (optional)',
		'planner.activityGroups': 'Groups',
		'planner.addActivity': 'Add Activity',
		'planner.editActivity': 'Edit Activity',
		'planner.saveActivity': 'Save',
		'planner.deleteActivity': 'Delete',
		'planner.cancelActivity': 'Cancel',
		'planner.activityNameRequired': 'Please enter an activity name.',
		'planner.activityBackToPlanner': 'Back to Planner',
		'categories.title': 'Manage Categories',
		'categories.loading': 'Loading categories...',
		'categories.noCategories': 'No categories yet.',
		'categories.name': 'Name',
		'categories.description': 'Description',
		'categories.color': 'Color',
		'categories.habits': 'Habits',
		'categories.save': 'Save',
		'categories.delete': 'Delete',
		'categories.cancel': 'Cancel',
		'categories.confirmDeleteTitle': 'Delete Category',
		'categories.confirmDeleteWithHabits': 'This category has habits. Choose what to do with them:',
		'categories.reassignTo': 'Move habits to another category',
		'categories.deleteHabits': 'Delete all habits in this category',
		'categories.confirmDeleteHabits': 'Are you sure? This will permanently delete all habits in this category.',
		'categories.deleted': 'Category deleted',
		'categories.saved': 'Category saved',
		'categories.selectCategory': 'Select a category',
		'nav.gratitude': 'Gratitude',
		'nav.activityLog': 'Activity Log',
		'gratitude.title': 'Gratitude Diary',
		'gratitude.loading': 'Loading gratitude entries...',
		'gratitude.noEntries': 'No gratitude entries yet. Write down something you are grateful for.',
		'gratitude.addEntry': 'Add Entry',
		'gratitude.placeholder': 'What are you grateful for today?',
		'gratitude.promptGrateful': "I'm grateful for...",
		'gratitude.promptThankful': "I'm thankful for...",
		'gratitude.because': 'because',
		'gratitude.reasonPlaceholder': 'Because... (optional)',
		'gratitude.todayCount': 'Today',
		'gratitude.weekCount': 'This Week',
		'gratitude.monthCount': 'This Month',
		'gratitude.dailyAverage': 'Daily Avg',
		'gratitude.currentStreak': 'Streak',
		'gratitude.streakDays': 'days'
	},
	de: {
		'nav.home': 'Start',
		'nav.edit': 'Bearbeiten',
		'nav.addHabit': 'Gewohnheit hinzufügen',
		'nav.addCategory': 'Kategorie hinzufügen',
		'nav.manageCategories': 'Kategorien',
		'nav.scoringRules': 'Bewertungsregeln',
		'nav.points': 'Punktediagramme',
		'nav.diary': 'Tagebuch',
		'nav.sleep': 'Schlaf',
		'nav.stats': 'Statistiken',
		'nav.emotions': 'Emotionen',
		'nav.exportPdf': 'PDF exportieren',
		'nav.backup': 'Sicherung',
		'nav.medication': 'Medikamente',
		'nav.translations': 'Übersetzungen',
		'nav.logout': 'Abmelden',
		'emotions.graphTitle': 'Emotionspaare-Diagramm',
		'emotions.week': 'Woche',
		'emotions.month': 'Monat',
		'emotions.loading': 'Emotionsdaten werden geladen...',
		'emotions.noPairs': 'Keine Emotionspaare gefunden. Erstelle zuerst einige Emotionspaare.',
		'emotions.avg': 'Durchschn.',
		'emotions.entries': 'Einträge',
		'emotions.positive': 'Positiv',
		'emotions.negative': 'Negativ',
		'emotions.avgLine': 'Durchschnitt',
		'home.title': 'Deine Gewohnheiten',
		'home.loading': 'Gewohnheiten werden geladen...',
		'home.noHabits': 'Noch keine Gewohnheiten.',
		'home.loadDefaults': 'Standardgewohnheiten laden',
		'home.loadingDefaults': 'Standards werden geladen...',
		'home.defaultsLoaded': 'Standardgewohnheiten erfolgreich geladen',
		'home.allCategories': 'Alle Kategorien',
		'home.complete': 'Erledigt',
		'home.undo': 'Rückgängig',
		'home.delete': 'Löschen',
		'home.confirmDelete': 'Möchtest du diese Gewohnheit wirklich löschen?',
		'home.todayEntries': 'heute',
		'home.dailyLimit': 'Tageslimit erreicht',
		'eval.title': 'Auswertung',
		'eval.streak': 'Aktuelle Serie',
		'eval.longestStreak': 'Längste Serie',
		'eval.completionRate': 'Abschlussrate',
		'eval.completions': 'Abschlüsse',
		'eval.target': 'Ziel',
		'eval.days': 'Tage',
		'eval.onTrack': 'Im Plan',
		'eval.behind': 'Rückstand',
		'eval.weeklyScore': 'Wochenpunktzahl',
		'eval.currentScore': 'Aktuell',
		'eval.predictedScore': 'Voraussichtlich',
		'eval.points': 'Pkt',
		'eval.avoidanceRate': 'Vermeidungsrate',
		'eval.avoidanceStreak': 'Vermeidungsserie',
		'firstStart.title': 'Willkommen bei Habit Evaluator',
		'firstStart.notProfessionalHelp': 'Diese App ist ein Selbstbeobachtungswerkzeug und ersetzt keine professionelle medizinische oder psychologische Hilfe. Wenn Sie Unterstützung brauchen, wenden Sie sich bitte an eine qualifizierte Fachperson.',
		'firstStart.noDataSharing': 'Standardmäßig bleiben alle Ihre Daten auf Ihrem Gerät und werden mit niemandem geteilt. Sie können optional einen Remote-Speicher in den Einstellungen konfigurieren.',
		'firstStart.acknowledge': 'Ich verstehe',
		'nav.imprint': 'Rechtliches',
		'imprint.title': 'Rechtliches',
		'imprint.contact': 'Kontakt',
		'imprint.name': 'Name',
		'imprint.email': 'E-Mail',
		'imprint.license': 'Lizenz',
		'imprint.licenseText': 'Dieses Projekt ist unter der MIT-Lizenz lizenziert.',
		'imprint.dataProtection': 'Datenschutz',
		'imprint.noRemoteStorage': 'Keine Speicherung von Daten auf einem Server, sofern nicht ausdrücklich vom Benutzer aktiviert.',
		'imprint.noTracking': 'Kein Tracking.',
		'imprint.noDataUsage': 'Keine Nutzung bereitgestellter Daten außer zur Bereitstellung des Dienstes.',
		'imprint.disclaimers': 'Haftungsausschlüsse',
		'imprint.notProfessionalHelp': 'Diese App ist ein Selbstbeobachtungswerkzeug und ersetzt keine professionelle medizinische oder psychologische Hilfe. Wenn Sie Unterstützung brauchen, wenden Sie sich bitte an eine qualifizierte Fachperson.',
		'imprint.noDataSharing': 'Standardmäßig bleiben alle Ihre Daten auf Ihrem Gerät und werden mit niemandem geteilt. Sie können optional einen Remote-Speicher in den Einstellungen konfigurieren.',
		'imprint.variantDisclaimer': 'Die Autoren dieses Projekts übernehmen keine Verantwortung für Änderungen durch Drittvarianten oder das Handeln von Webservice-Hostern.',
		'imprint.libraries': 'Drittanbieter-Bibliotheken',
		'imprint.libraryName': 'Bibliothek',
		'imprint.libraryVersion': 'Version',
		'imprint.libraryLicense': 'Lizenz',
		'correlations.title': 'Ereignis-Korrelationen',
		'correlations.description': 'Zeitgewichtete Pearson-Korrelationen zwischen Gewohnheiten, Tagebuch, Schlaf und Emotionen im vergangenen Jahr. Neuere Daten werden stärker gewichtet.',
		'correlations.eventA': 'Ereignis A',
		'correlations.eventB': 'Ereignis B',
		'correlations.correlation': 'Korrelation',
		'correlations.sharedDays': 'Gemeinsame Tage',
		'correlations.confidence': 'Konfidenz',
		'correlations.strong': 'Stark',
		'correlations.moderate': 'Mittel',
		'correlations.weak': 'Schwach',
		'correlations.loading': 'Korrelationen werden geladen...',
		'correlations.noData': 'Keine Korrelationen gefunden. Es werden mindestens 7 Tage überlappende Daten zwischen zwei Ereignissen benötigt.',
		'correlations.disclaimer': 'Korrelation bedeutet nicht Kausalität. Schwache Korrelationen (|r| < 0,3) haben eine niedrige statistische Konfidenz und sollten mit Vorsicht interpretiert werden.',
		'correlations.backToStats': 'Zurück zu Statistiken',
		'nav.correlations': 'Korrelationen',
		'nav.settings': 'Einstellungen',
		'nav.planner': 'Planer',
		'nav.foodLog': 'Ernährungsprotokoll',
		'nav.sportGraph': 'Sportdiagramm',
		'nav.foodDistribution': 'Ernährungsgrafik',
		'foodDistribution.title': 'Tägliche Essensverteilung',
		'foodDistribution.description': 'Mahlzeitenhäufigkeit und durchschnittliche Nährwerte nach Tageszeit der letzten 30 Tage.',
		'foodDistribution.loading': 'Essensverteilung wird geladen...',
		'foodDistribution.noData': 'Keine Ernährungseinträge in den letzten 30 Tagen.',
		'foodDistribution.mealsPerHour': 'Mahlzeiten pro Stunde',
		'foodDistribution.avgKcalPerHour': 'Durchschn. Kcal pro Stunde',
		'foodDistribution.avgCarbsPerHour': 'Durchschn. Kohlenhydrate (g) pro Stunde',
		'foodDistribution.meals': 'Mahlzeiten',
		'foodDistribution.totalMeals': 'Mahlzeiten gesamt (30 Tage)',
		'foodDistribution.backToFoodLog': 'Zurück zum Ernährungsprotokoll',
		'settings.moduleVisibility': 'Modulsichtbarkeit',
		'settings.moduleVisibilityHint': 'Blenden Sie nicht benötigte Module aus, um die Navigation zu vereinfachen.',
		'settings.moduleDiary': 'Tagebuch',
		'settings.moduleSleep': 'Schlaftracking',
		'settings.moduleEmotions': 'Emotionen',
		'settings.modulePoints': 'Punktediagramme',
		'settings.moduleStatistics': 'Statistiken',
		'settings.moduleFoodLog': 'Ernährungsprotokoll',
		'settings.moduleSportLog': 'Sportprotokoll',
		'settings.moduleMedication': 'Medikamente',
		'settings.moduleBackup': 'Sicherung',
		'settings.modulePdfExport': 'PDF-Export',
		'settings.moduleDayPlanner': 'Planer',
		'planner.weekPlanner': 'Wochenplaner',
		'planner.weekPlannerDescription': 'Planen Sie Ihre Woche mit Aktivitätsgruppen und Zeitfenstern.',
		'planner.weekOverview': 'Wochenübersicht',
		'planner.weekOverviewDescription': 'Visuelle Übersicht der belegten Zeitfenster in der Woche.',
		'planner.weekOverviewLoading': 'Wochenübersicht wird geladen...',
		'planner.weekOverviewEmpty': 'Noch keine Zeitfenster zugewiesen. Verwenden Sie den Wochenplaner, um Aktivitätsgruppen Zeitfenstern zuzuordnen.',
		'planner.weekOverviewBackToPlanner': 'Zurück zum Planer',
		'planner.weekOverviewOccupied': 'belegt',
		'planner.weekOverviewFree': 'frei',
		'planner.activities': 'Aktivitäten',
		'planner.activitiesDescription': 'Verwalten Sie Aktivitäten und ordnen Sie sie Gruppen zu.',
		'planner.groups': 'Aktivitätsgruppen',
		'planner.groupsDescription': 'Erstellen und verwalten Sie Gruppen zur Organisation Ihrer Aktivitäten.',
		'planner.groupsLoading': 'Gruppen werden geladen...',
		'planner.noGroups': 'Noch keine Gruppen. Erstellen Sie eine Gruppe.',
		'planner.noGroupActivities': 'Keine Aktivitäten in dieser Gruppe',
		'planner.groupName': 'Name',
		'planner.groupDescription': 'Beschreibung (optional)',
		'planner.addGroup': 'Gruppe hinzufügen',
		'planner.editGroup': 'Gruppe bearbeiten',
		'planner.saveGroup': 'Speichern',
		'planner.deleteGroup': 'Löschen',
		'planner.cancelGroup': 'Abbrechen',
		'planner.groupNameRequired': 'Bitte geben Sie einen Gruppennamen ein.',
		'planner.groupBackToPlanner': 'Zurück zum Planer',
		'planner.activitiesLoading': 'Aktivitäten werden geladen...',
		'planner.noActivities': 'Noch keine Aktivitäten. Erstellen Sie eine Aktivität.',
		'planner.activityName': 'Name',
		'planner.activityDescription': 'Beschreibung (optional)',
		'planner.activityGroups': 'Gruppen',
		'planner.addActivity': 'Aktivität hinzufügen',
		'planner.editActivity': 'Aktivität bearbeiten',
		'planner.saveActivity': 'Speichern',
		'planner.deleteActivity': 'Löschen',
		'planner.cancelActivity': 'Abbrechen',
		'planner.activityNameRequired': 'Bitte geben Sie einen Aktivitätsnamen ein.',
		'planner.activityBackToPlanner': 'Zurück zum Planer',
		'categories.title': 'Kategorien verwalten',
		'categories.loading': 'Kategorien werden geladen...',
		'categories.noCategories': 'Noch keine Kategorien.',
		'categories.name': 'Name',
		'categories.description': 'Beschreibung',
		'categories.color': 'Farbe',
		'categories.habits': 'Gewohnheiten',
		'categories.save': 'Speichern',
		'categories.delete': 'L\u00f6schen',
		'categories.cancel': 'Abbrechen',
		'categories.confirmDeleteTitle': 'Kategorie l\u00f6schen',
		'categories.confirmDeleteWithHabits': 'Diese Kategorie enth\u00e4lt Gewohnheiten. W\u00e4hlen Sie, was damit geschehen soll:',
		'categories.reassignTo': 'Gewohnheiten in eine andere Kategorie verschieben',
		'categories.deleteHabits': 'Alle Gewohnheiten in dieser Kategorie l\u00f6schen',
		'categories.confirmDeleteHabits': 'Sind Sie sicher? Alle Gewohnheiten in dieser Kategorie werden dauerhaft gel\u00f6scht.',
		'categories.deleted': 'Kategorie gel\u00f6scht',
		'categories.saved': 'Kategorie gespeichert',
		'categories.selectCategory': 'Kategorie ausw\u00e4hlen',
		'nav.gratitude': 'Dankbarkeit',
		'nav.activityLog': 'Aktivit\u00e4tsprotokoll',
		'gratitude.title': 'Dankbarkeitstagebuch',
		'gratitude.loading': 'Dankbarkeitseintr\u00e4ge werden geladen...',
		'gratitude.noEntries': 'Noch keine Dankbarkeitseintr\u00e4ge. Schreibe auf, wof\u00fcr du dankbar bist.',
		'gratitude.addEntry': 'Eintrag hinzuf\u00fcgen',
		'gratitude.placeholder': 'Wof\u00fcr bist du heute dankbar?',
		'gratitude.promptGrateful': 'Ich bin dankbar f\u00fcr...',
		'gratitude.promptThankful': 'Ich bin froh \u00fcber...',
		'gratitude.because': 'weil',
		'gratitude.reasonPlaceholder': 'Weil\u2026 (optional)',
		'gratitude.todayCount': 'Heute',
		'gratitude.weekCount': 'Diese Woche',
		'gratitude.monthCount': 'Dieser Monat',
		'gratitude.dailyAverage': 'Tagesschnitt',
		'gratitude.currentStreak': 'Serie',
		'gratitude.streakDays': 'Tage'
	},
	es: {
		'nav.home': 'Inicio',
		'nav.edit': 'Editar',
		'nav.addHabit': 'Añadir hábito',
		'nav.addCategory': 'A\u00f1adir categor\u00eda',
		'nav.manageCategories': 'Categor\u00edas',
		'nav.scoringRules': 'Reglas de puntuaci\u00f3n',
		'nav.points': 'Gráficos de puntos',
		'nav.diary': 'Diario',
		'nav.sleep': 'Sueño',
		'nav.stats': 'Estadísticas',
		'nav.emotions': 'Emociones',
		'nav.exportPdf': 'Exportar PDF',
		'nav.backup': 'Copia de seguridad',
		'nav.medication': 'Medicación',
		'nav.translations': 'Traducciones',
		'nav.logout': 'Cerrar sesión',
		'emotions.graphTitle': 'Gráfico de pares emocionales',
		'emotions.week': 'Semana',
		'emotions.month': 'Mes',
		'emotions.loading': 'Cargando datos emocionales...',
		'emotions.noPairs': 'No se encontraron pares emocionales. Crea algunos primero.',
		'emotions.avg': 'Prom.',
		'emotions.entries': 'Entradas',
		'emotions.positive': 'Positivo',
		'emotions.negative': 'Negativo',
		'emotions.avgLine': 'Promedio',
		'home.title': 'Tus hábitos',
		'home.loading': 'Cargando hábitos...',
		'home.noHabits': 'Aún no hay hábitos.',
		'home.loadDefaults': 'Cargar hábitos predeterminados',
		'home.loadingDefaults': 'Cargando predeterminados...',
		'home.defaultsLoaded': 'Hábitos predeterminados cargados con éxito',
		'home.allCategories': 'Todas las categorías',
		'home.complete': 'Completar',
		'home.undo': 'Deshacer',
		'home.delete': 'Eliminar',
		'home.confirmDelete': '¿Estás seguro de que quieres eliminar este hábito?',
		'home.todayEntries': 'hoy',
		'home.dailyLimit': 'Límite diario alcanzado',
		'eval.title': 'Evaluación',
		'eval.streak': 'Racha actual',
		'eval.longestStreak': 'Racha más larga',
		'eval.completionRate': 'Tasa de finalización',
		'eval.completions': 'Finalizaciones',
		'eval.target': 'Objetivo',
		'eval.days': 'días',
		'eval.onTrack': 'En camino',
		'eval.behind': 'Atrasado',
		'eval.weeklyScore': 'Puntuación semanal',
		'eval.currentScore': 'Actual',
		'eval.predictedScore': 'Previsto',
		'eval.points': 'pts',
		'eval.avoidanceRate': 'Tasa de evitación',
		'eval.avoidanceStreak': 'Racha de evitación',
		'firstStart.title': 'Bienvenido a Habit Evaluator',
		'firstStart.notProfessionalHelp': 'Esta aplicación es una herramienta de autoseguimiento y no reemplaza la ayuda profesional médica o psicológica. Si necesita apoyo, consulte a un profesional cualificado.',
		'firstStart.noDataSharing': 'Por defecto, todos sus datos permanecen en su dispositivo y no se comparten con nadie. Opcionalmente, puede configurar almacenamiento remoto en los ajustes.',
		'firstStart.acknowledge': 'Entendido',
		'nav.imprint': 'Legal del proyecto',
		'imprint.title': 'Legal del proyecto',
		'imprint.contact': 'Contacto',
		'imprint.name': 'Nombre',
		'imprint.email': 'Correo electr\u00f3nico',
		'imprint.license': 'Licencia',
		'imprint.licenseText': 'Este proyecto est\u00e1 licenciado bajo la Licencia MIT.',
		'imprint.dataProtection': 'Protecci\u00f3n de datos',
		'imprint.noRemoteStorage': 'No se almacenan datos de forma remota a menos que el usuario lo habilite expl\u00edcitamente.',
		'imprint.noTracking': 'Sin rastreo.',
		'imprint.noDataUsage': 'No se utilizan los datos proporcionados m\u00e1s all\u00e1 de la prestaci\u00f3n del servicio.',
		'imprint.disclaimers': 'Descargos de responsabilidad',
		'imprint.notProfessionalHelp': 'Esta aplicaci\u00f3n es una herramienta de autoseguimiento y no reemplaza la ayuda profesional m\u00e9dica o psicol\u00f3gica. Si necesita apoyo, consulte a un profesional cualificado.',
		'imprint.noDataSharing': 'Por defecto, todos sus datos permanecen en su dispositivo y no se comparten con nadie. Opcionalmente, puede configurar almacenamiento remoto en los ajustes.',
		'imprint.variantDisclaimer': 'Los autores de este proyecto no son responsables de las modificaciones realizadas por variantes de terceros ni de las acciones de los proveedores de servicios web.',
		'imprint.libraries': 'Bibliotecas de terceros',
		'imprint.libraryName': 'Biblioteca',
		'imprint.libraryVersion': 'Versi\u00f3n',
		'imprint.libraryLicense': 'Licencia',
		'correlations.title': 'Correlaciones de eventos',
		'correlations.description': 'Correlaciones de Pearson ponderadas por tiempo entre h\u00e1bitos, diario, sue\u00f1o y emociones durante el \u00faltimo a\u00f1o. Los datos m\u00e1s recientes tienen mayor peso.',
		'correlations.eventA': 'Evento A',
		'correlations.eventB': 'Evento B',
		'correlations.correlation': 'Correlaci\u00f3n',
		'correlations.sharedDays': 'D\u00edas compartidos',
		'correlations.confidence': 'Confianza',
		'correlations.strong': 'Fuerte',
		'correlations.moderate': 'Moderada',
		'correlations.weak': 'D\u00e9bil',
		'correlations.loading': 'Cargando correlaciones...',
		'correlations.noData': 'No se encontraron correlaciones. Se necesitan al menos 7 d\u00edas de datos superpuestos entre dos eventos.',
		'correlations.disclaimer': 'Correlaci\u00f3n no implica causalidad. Las correlaciones d\u00e9biles (|r| < 0,3) tienen baja confianza estad\u00edstica y deben interpretarse con precauci\u00f3n.',
		'correlations.backToStats': 'Volver a Estad\u00edsticas',
		'nav.correlations': 'Correlaciones',
		'nav.settings': 'Ajustes',
		'nav.planner': 'Planificador',
		'nav.foodLog': 'Registro de comidas',
		'nav.sportGraph': 'Gr\u00e1fico deportivo',
		'nav.foodDistribution': 'Gráfico de comidas',
		'foodDistribution.title': 'Distribución diaria de comidas',
		'foodDistribution.description': 'Frecuencia de comidas y promedios nutricionales por hora del día en los últimos 30 días.',
		'foodDistribution.loading': 'Cargando distribución de comidas...',
		'foodDistribution.noData': 'No hay entradas de comida en los últimos 30 días.',
		'foodDistribution.mealsPerHour': 'Comidas por hora',
		'foodDistribution.avgKcalPerHour': 'Kcal promedio por hora',
		'foodDistribution.avgCarbsPerHour': 'Carbohidratos promedio (g) por hora',
		'foodDistribution.meals': 'comidas',
		'foodDistribution.totalMeals': 'Total de comidas (30 días)',
		'foodDistribution.backToFoodLog': 'Volver al registro de comidas',
		'settings.moduleVisibility': 'Visibilidad de módulos',
		'settings.moduleVisibilityHint': 'Oculte módulos que no usa para simplificar la navegación.',
		'settings.moduleDiary': 'Diario',
		'settings.moduleSleep': 'Seguimiento del sueño',
		'settings.moduleEmotions': 'Emociones',
		'settings.modulePoints': 'Gráficos de puntos',
		'settings.moduleStatistics': 'Estadísticas',
		'settings.moduleFoodLog': 'Registro de comidas',
		'settings.moduleSportLog': 'Registro deportivo',
		'settings.moduleMedication': 'Medicación',
		'settings.moduleBackup': 'Copia de seguridad',
		'settings.modulePdfExport': 'Exportar PDF',
		'settings.moduleDayPlanner': 'Planificador',
		'planner.weekPlanner': 'Planificador semanal',
		'planner.weekPlannerDescription': 'Planifique su semana asignando grupos de actividades a franjas horarias.',
		'planner.weekOverview': 'Resumen semanal',
		'planner.weekOverviewDescription': 'Vista visual de las franjas horarias ocupadas durante la semana.',
		'planner.weekOverviewLoading': 'Cargando resumen semanal...',
		'planner.weekOverviewEmpty': 'A\u00fan no hay franjas horarias asignadas. Use el planificador semanal para asignar grupos de actividades a franjas horarias.',
		'planner.weekOverviewBackToPlanner': 'Volver al planificador',
		'planner.weekOverviewOccupied': 'ocupado',
		'planner.weekOverviewFree': 'libre',
		'planner.activities': 'Actividades',
		'planner.activitiesDescription': 'Gestione actividades y asígnelas a grupos.',
		'planner.groups': 'Grupos de actividades',
		'planner.groupsDescription': 'Cree y gestione grupos para organizar sus actividades.',
		'planner.groupsLoading': 'Cargando grupos...',
		'planner.noGroups': 'A\u00fan no hay grupos. Cree un grupo para empezar.',
		'planner.noGroupActivities': 'No hay actividades en este grupo',
		'planner.groupName': 'Nombre',
		'planner.groupDescription': 'Descripci\u00f3n (opcional)',
		'planner.addGroup': 'A\u00f1adir grupo',
		'planner.editGroup': 'Editar grupo',
		'planner.saveGroup': 'Guardar',
		'planner.deleteGroup': 'Eliminar',
		'planner.cancelGroup': 'Cancelar',
		'planner.groupNameRequired': 'Por favor, introduzca un nombre de grupo.',
		'planner.groupBackToPlanner': 'Volver al planificador',
		'planner.activitiesLoading': 'Cargando actividades...',
		'planner.noActivities': 'A\u00fan no hay actividades. Cree una actividad para empezar.',
		'planner.activityName': 'Nombre',
		'planner.activityDescription': 'Descripci\u00f3n (opcional)',
		'planner.activityGroups': 'Grupos',
		'planner.addActivity': 'A\u00f1adir actividad',
		'planner.editActivity': 'Editar actividad',
		'planner.saveActivity': 'Guardar',
		'planner.deleteActivity': 'Eliminar',
		'planner.cancelActivity': 'Cancelar',
		'planner.activityNameRequired': 'Por favor, introduzca un nombre de actividad.',
		'planner.activityBackToPlanner': 'Volver al planificador',
		'categories.title': 'Gestionar categor\u00edas',
		'categories.loading': 'Cargando categor\u00edas...',
		'categories.noCategories': 'A\u00fan no hay categor\u00edas.',
		'categories.name': 'Nombre',
		'categories.description': 'Descripci\u00f3n',
		'categories.color': 'Color',
		'categories.habits': 'H\u00e1bitos',
		'categories.save': 'Guardar',
		'categories.delete': 'Eliminar',
		'categories.cancel': 'Cancelar',
		'categories.confirmDeleteTitle': 'Eliminar categor\u00eda',
		'categories.confirmDeleteWithHabits': 'Esta categor\u00eda tiene h\u00e1bitos. Elige qu\u00e9 hacer con ellos:',
		'categories.reassignTo': 'Mover h\u00e1bitos a otra categor\u00eda',
		'categories.deleteHabits': 'Eliminar todos los h\u00e1bitos de esta categor\u00eda',
		'categories.confirmDeleteHabits': '\u00bfEst\u00e1s seguro? Esto eliminar\u00e1 permanentemente todos los h\u00e1bitos de esta categor\u00eda.',
		'categories.deleted': 'Categor\u00eda eliminada',
		'categories.saved': 'Categor\u00eda guardada',
		'categories.selectCategory': 'Seleccionar una categor\u00eda',
		'nav.gratitude': 'Gratitud',
		'nav.activityLog': 'Registro de actividades',
		'gratitude.title': 'Diario de gratitud',
		'gratitude.loading': 'Cargando entradas de gratitud...',
		'gratitude.noEntries': 'A\u00fan no hay entradas de gratitud. Escribe algo por lo que est\u00e9s agradecido.',
		'gratitude.addEntry': 'A\u00f1adir entrada',
		'gratitude.placeholder': '\u00bfPor qu\u00e9 est\u00e1s agradecido hoy?',
		'gratitude.promptGrateful': 'Estoy agradecido por...',
		'gratitude.promptThankful': 'Doy gracias por...',
		'gratitude.because': 'porque',
		'gratitude.reasonPlaceholder': 'Porque\u2026 (opcional)',
		'gratitude.todayCount': 'Hoy',
		'gratitude.weekCount': 'Esta semana',
		'gratitude.monthCount': 'Este mes',
		'gratitude.dailyAverage': 'Promedio diario',
		'gratitude.currentStreak': 'Racha',
		'gratitude.streakDays': 'd\u00edas'
	},
	fr: {
		'nav.home': 'Accueil',
		'nav.edit': 'Modifier',
		'nav.addHabit': 'Ajouter habitude',
		'nav.addCategory': 'Ajouter cat\u00e9gorie',
		'nav.manageCategories': 'Cat\u00e9gories',
		'nav.scoringRules': 'R\u00e8gles de notation',
		'nav.points': 'Graphiques de points',
		'nav.diary': 'Journal',
		'nav.sleep': 'Sommeil',
		'nav.stats': 'Statistiques',
		'nav.emotions': 'Émotions',
		'nav.exportPdf': 'Exporter PDF',
		'nav.backup': 'Sauvegarde',
		'nav.medication': 'Médicaments',
		'nav.translations': 'Traductions',
		'nav.logout': 'Déconnexion',
		'emotions.graphTitle': 'Graphique des paires émotionnelles',
		'emotions.week': 'Semaine',
		'emotions.month': 'Mois',
		'emotions.loading': 'Chargement des données émotionnelles...',
		'emotions.noPairs': "Aucune paire émotionnelle trouvée. Créez-en d'abord.",
		'emotions.avg': 'Moy.',
		'emotions.entries': 'Entrées',
		'emotions.positive': 'Positif',
		'emotions.negative': 'Négatif',
		'emotions.avgLine': 'Moyenne',
		'home.title': 'Vos habitudes',
		'home.loading': 'Chargement des habitudes...',
		'home.noHabits': "Pas encore d'habitudes.",
		'home.loadDefaults': 'Charger les habitudes par défaut',
		'home.loadingDefaults': 'Chargement des défauts...',
		'home.defaultsLoaded': 'Habitudes par défaut chargées avec succès',
		'home.allCategories': 'Toutes les catégories',
		'home.complete': 'Compléter',
		'home.undo': 'Annuler',
		'home.delete': 'Supprimer',
		'home.confirmDelete': 'Êtes-vous sûr de vouloir supprimer cette habitude ?',
		'home.todayEntries': "aujourd'hui",
		'home.dailyLimit': 'Limite journalière atteinte',
		'eval.title': 'Évaluation',
		'eval.streak': 'Série actuelle',
		'eval.longestStreak': 'Plus longue série',
		'eval.completionRate': "Taux d'achèvement",
		'eval.completions': 'Achèvements',
		'eval.target': 'Objectif',
		'eval.days': 'jours',
		'eval.onTrack': 'En bonne voie',
		'eval.behind': 'En retard',
		'eval.weeklyScore': 'Score hebdomadaire',
		'eval.currentScore': 'Actuel',
		'eval.predictedScore': 'Prévu',
		'eval.points': 'pts',
		'eval.avoidanceRate': "Taux d'évitement",
		'eval.avoidanceStreak': "Série d'évitement",
		'firstStart.title': 'Bienvenue sur Habit Evaluator',
		'firstStart.notProfessionalHelp': "Cette application est un outil de suivi personnel et ne remplace pas une aide médicale ou psychologique professionnelle. Si vous avez besoin de soutien, veuillez consulter un professionnel qualifié.",
		'firstStart.noDataSharing': "Par défaut, toutes vos données restent sur votre appareil et ne sont partagées avec personne. Vous pouvez éventuellement configurer un stockage distant dans les paramètres.",
		'firstStart.acknowledge': 'Je comprends',
		'nav.imprint': 'L\u00e9gal du projet',
		'imprint.title': 'L\u00e9gal du projet',
		'imprint.contact': 'Contact',
		'imprint.name': 'Nom',
		'imprint.email': 'E-mail',
		'imprint.license': 'Licence',
		'imprint.licenseText': 'Ce projet est sous licence MIT.',
		'imprint.dataProtection': 'Protection des donn\u00e9es',
		'imprint.noRemoteStorage': "Aucun stockage distant des donn\u00e9es sauf activation explicite par l'utilisateur.",
		'imprint.noTracking': 'Aucun suivi.',
		'imprint.noDataUsage': "Aucune utilisation des donn\u00e9es fournies en dehors de la fourniture du service.",
		'imprint.disclaimers': 'Avertissements',
		'imprint.notProfessionalHelp': "Cette application est un outil de suivi personnel et ne remplace pas une aide m\u00e9dicale ou psychologique professionnelle. Si vous avez besoin de soutien, veuillez consulter un professionnel qualifi\u00e9.",
		'imprint.noDataSharing': "Par d\u00e9faut, toutes vos donn\u00e9es restent sur votre appareil et ne sont partag\u00e9es avec personne. Vous pouvez \u00e9ventuellement configurer un stockage distant dans les param\u00e8tres.",
		'imprint.variantDisclaimer': "Les auteurs de ce projet ne sont pas responsables des modifications apport\u00e9es par des variantes tierces ni des actions des h\u00e9bergeurs de services web.",
		'imprint.libraries': 'Biblioth\u00e8ques tierces',
		'imprint.libraryName': 'Biblioth\u00e8que',
		'imprint.libraryVersion': 'Version',
		'imprint.libraryLicense': 'Licence',
		'correlations.title': 'Corr\u00e9lations d\'\u00e9v\u00e9nements',
		'correlations.description': 'Corr\u00e9lations de Pearson pond\u00e9r\u00e9es dans le temps entre habitudes, journal, sommeil et \u00e9motions au cours de l\'ann\u00e9e pass\u00e9e. Les donn\u00e9es r\u00e9centes ont un poids plus \u00e9lev\u00e9.',
		'correlations.eventA': '\u00c9v\u00e9nement A',
		'correlations.eventB': '\u00c9v\u00e9nement B',
		'correlations.correlation': 'Corr\u00e9lation',
		'correlations.sharedDays': 'Jours partag\u00e9s',
		'correlations.confidence': 'Confiance',
		'correlations.strong': 'Forte',
		'correlations.moderate': 'Mod\u00e9r\u00e9e',
		'correlations.weak': 'Faible',
		'correlations.loading': 'Chargement des corr\u00e9lations...',
		'correlations.noData': 'Aucune corr\u00e9lation trouv\u00e9e. Il faut au moins 7 jours de donn\u00e9es communes entre deux \u00e9v\u00e9nements.',
		'correlations.disclaimer': 'Corr\u00e9lation n\'implique pas causalit\u00e9. Les corr\u00e9lations faibles (|r| < 0,3) ont une faible confiance statistique et doivent \u00eatre interpr\u00e9t\u00e9es avec prudence.',
		'correlations.backToStats': 'Retour aux Statistiques',
		'nav.correlations': 'Corr\u00e9lations',
		'nav.settings': 'Param\u00e8tres',
		'nav.planner': 'Planificateur',
		'nav.foodLog': 'Journal alimentaire',
		'nav.sportGraph': 'Graphique sportif',
		'nav.foodDistribution': 'Graphique alimentaire',
		'foodDistribution.title': 'Distribution alimentaire quotidienne',
		'foodDistribution.description': 'Fréquence des repas et moyennes nutritionnelles par heure sur les 30 derniers jours.',
		'foodDistribution.loading': 'Chargement de la distribution alimentaire...',
		'foodDistribution.noData': 'Aucune entrée alimentaire au cours des 30 derniers jours.',
		'foodDistribution.mealsPerHour': 'Repas par heure',
		'foodDistribution.avgKcalPerHour': 'Kcal moyen par heure',
		'foodDistribution.avgCarbsPerHour': 'Glucides moyens (g) par heure',
		'foodDistribution.meals': 'repas',
		'foodDistribution.totalMeals': 'Total des repas (30 jours)',
		'foodDistribution.backToFoodLog': 'Retour au journal alimentaire',
		'settings.moduleVisibility': 'Visibilité des modules',
		'settings.moduleVisibilityHint': 'Masquez les modules inutilisés pour simplifier la navigation.',
		'settings.moduleDiary': 'Journal',
		'settings.moduleSleep': 'Suivi du sommeil',
		'settings.moduleEmotions': 'Émotions',
		'settings.modulePoints': 'Graphiques de points',
		'settings.moduleStatistics': 'Statistiques',
		'settings.moduleFoodLog': 'Journal alimentaire',
		'settings.moduleSportLog': 'Journal sportif',
		'settings.moduleMedication': 'Médicaments',
		'settings.moduleBackup': 'Sauvegarde',
		'settings.modulePdfExport': 'Export PDF',
		'settings.moduleDayPlanner': 'Planificateur',
		'planner.weekPlanner': 'Planificateur hebdomadaire',
		'planner.weekPlannerDescription': "Planifiez votre semaine en attribuant des groupes d'activités à des créneaux horaires.",
		'planner.weekOverview': 'Aper\u00e7u de la semaine',
		'planner.weekOverviewDescription': "Aper\u00e7u visuel des cr\u00e9neaux horaires occup\u00e9s pendant la semaine.",
		'planner.weekOverviewLoading': "Chargement de l'aper\u00e7u de la semaine...",
		'planner.weekOverviewEmpty': "Aucun cr\u00e9neau horaire attribu\u00e9. Utilisez le planificateur hebdomadaire pour attribuer des groupes d'activit\u00e9s \u00e0 des cr\u00e9neaux.",
		'planner.weekOverviewBackToPlanner': 'Retour au planificateur',
		'planner.weekOverviewOccupied': 'occup\u00e9',
		'planner.weekOverviewFree': 'libre',
		'planner.activities': 'Activités',
		'planner.activitiesDescription': 'Gérez les activités et assignez-les à des groupes.',
		'planner.groups': "Groupes d'activit\u00e9s",
		'planner.groupsDescription': 'Cr\u00e9ez et g\u00e9rez des groupes pour organiser vos activit\u00e9s.',
		'planner.groupsLoading': 'Chargement des groupes...',
		'planner.noGroups': "Aucun groupe pour le moment. Cr\u00e9ez un groupe pour commencer.",
		'planner.noGroupActivities': 'Aucune activité dans ce groupe',
		'planner.groupName': 'Nom',
		'planner.groupDescription': 'Description (facultative)',
		'planner.addGroup': 'Ajouter un groupe',
		'planner.editGroup': 'Modifier le groupe',
		'planner.saveGroup': 'Enregistrer',
		'planner.deleteGroup': 'Supprimer',
		'planner.cancelGroup': 'Annuler',
		'planner.groupNameRequired': "Veuillez entrer un nom de groupe.",
		'planner.groupBackToPlanner': 'Retour au planificateur',
		'planner.activitiesLoading': 'Chargement des activit\u00e9s...',
		'planner.noActivities': "Aucune activit\u00e9 pour le moment. Cr\u00e9ez une activit\u00e9 pour commencer.",
		'planner.activityName': 'Nom',
		'planner.activityDescription': 'Description (facultative)',
		'planner.activityGroups': 'Groupes',
		'planner.addActivity': 'Ajouter une activit\u00e9',
		'planner.editActivity': "Modifier l'activit\u00e9",
		'planner.saveActivity': 'Enregistrer',
		'planner.deleteActivity': 'Supprimer',
		'planner.cancelActivity': 'Annuler',
		'planner.activityNameRequired': "Veuillez entrer un nom d'activit\u00e9.",
		'planner.activityBackToPlanner': 'Retour au planificateur',
		'categories.title': 'G\u00e9rer les cat\u00e9gories',
		'categories.loading': 'Chargement des cat\u00e9gories...',
		'categories.noCategories': 'Aucune cat\u00e9gorie pour le moment.',
		'categories.name': 'Nom',
		'categories.description': 'Description',
		'categories.color': 'Couleur',
		'categories.habits': 'Habitudes',
		'categories.save': 'Enregistrer',
		'categories.delete': 'Supprimer',
		'categories.cancel': 'Annuler',
		'categories.confirmDeleteTitle': 'Supprimer la cat\u00e9gorie',
		'categories.confirmDeleteWithHabits': 'Cette cat\u00e9gorie contient des habitudes. Choisissez quoi en faire :',
		'categories.reassignTo': 'D\u00e9placer les habitudes vers une autre cat\u00e9gorie',
		'categories.deleteHabits': 'Supprimer toutes les habitudes de cette cat\u00e9gorie',
		'categories.confirmDeleteHabits': '\u00cates-vous s\u00fbr ? Toutes les habitudes de cette cat\u00e9gorie seront d\u00e9finitivement supprim\u00e9es.',
		'categories.deleted': 'Cat\u00e9gorie supprim\u00e9e',
		'categories.saved': 'Cat\u00e9gorie enregistr\u00e9e',
		'categories.selectCategory': 'S\u00e9lectionner une cat\u00e9gorie',
		'nav.gratitude': 'Gratitude',
		'nav.activityLog': 'Journal d\'activit\u00e9s',
		'gratitude.title': 'Journal de gratitude',
		'gratitude.loading': 'Chargement des entr\u00e9es de gratitude...',
		'gratitude.noEntries': 'Aucune entr\u00e9e de gratitude. \u00c9crivez quelque chose pour lequel vous \u00eates reconnaissant.',
		'gratitude.addEntry': 'Ajouter une entr\u00e9e',
		'gratitude.placeholder': 'Pour quoi \u00eates-vous reconnaissant aujourd\'hui ?',
		'gratitude.promptGrateful': 'Je suis reconnaissant pour...',
		'gratitude.promptThankful': 'Je remercie pour...',
		'gratitude.because': 'parce que',
		'gratitude.reasonPlaceholder': 'Parce que\u2026 (facultatif)',
		'gratitude.todayCount': 'Aujourd\'hui',
		'gratitude.weekCount': 'Cette semaine',
		'gratitude.monthCount': 'Ce mois',
		'gratitude.dailyAverage': 'Moyenne quotidienne',
		'gratitude.currentStreak': 'S\u00e9rie',
		'gratitude.streakDays': 'jours'
	}
};

export function getLanguage(): Language {
	if (typeof localStorage !== 'undefined') {
		const saved = localStorage.getItem('language');
		if (saved && (saved === 'en' || saved === 'de' || saved === 'es' || saved === 'fr')) {
			return saved as Language;
		}
	}
	return 'en';
}

export function setLanguage(lang: Language): void {
	localStorage.setItem('language', lang);
}

export function t(key: string, lang: Language): string {
	return translations[lang]?.[key] ?? translations['en']?.[key] ?? key;
}
