<script lang="ts">
	...
	async function handleAdd(e: Event) {
		...
		error = '';
		try {
			const createPayload: { persons: string; location: string; startTime: string; endTime: string; date: string; activity?: string } = {
			...
			if (activity.trim()) createPayload.activity = activity.trim();
			await activityLogs.create(createPayload);
			persons = '';
			location = '';
			startTime = '';
			endTime = '';
			...
			error = err instanceof Error ? err.message : 'Failed to add activity log entry';
		}
	}
