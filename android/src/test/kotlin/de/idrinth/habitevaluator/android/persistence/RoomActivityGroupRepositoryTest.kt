...
class RoomActivityGroupRepositoryTest {

    private lateinit var dao: ActivityLogDao
    private lateinit var repository: RoomActivityGroupRepository

    @BeforeEach
    fun setUp() {
        dao = mock(ActivityLogDao::class.java)
        repository = RoomActivityGroupRepository(dao)
    }
    ...
    fun testSaveReturnsGroup() = runTest {
        ...
        assertEquals("Social", result.name)
        verify(dao).insertGroup(any(ActivityGroupEntity::class.java) ?: createGroupEntity())
    }

    @Test
    fun testFindByIdReturnsGroup() = runTest {
        val entity = createGroupEntity(description = "Social activities")
        `when`(dao.findGroupById("g1")).thenReturn(entity)

        val found = repository.findById("g1")
        assertTrue(found.isPresent)
        assertEquals("Social", found.get().name)
        assertEquals("Social activities", found.get().description)
    }
    ...
    fun testFindAll() = runTest {
        val e1 = createGroupEntity(id = "g1", name = "Social")
        val e2 = createGroupEntity(id = "g2", name = "Work")
    ...
    fun testFindByUserId() = runTest {
        val e1 = createGroupEntity(id = "g1", name = "Social", userId = "u1")
        val e2 = createGroupEntity(id = "g2", name = "Work", userId = "u1")
