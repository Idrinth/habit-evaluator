...
- **Route structure:**
  ...
  - `src/routes/main/` — 38 benchmark routes covering all API endpoints (auth, habits, categories, diary, sleep, emotions, food-logs, sport-logs, medications, meetings, stats, sync, magic-links, reminder-settings, score-rules, activity-logs)
...
- Screens (Android Compose): `*Screen` (`HomeScreen`, `DiaryScreen`, `EditHabitsScreen`, `StatsScreen`, `SettingsScreen`, `FoodLogScreen`, `SportLogScreen`, `MedicationListScreen`, `MedicationLogScreen`, `EmergencyPlanScreen`, `ActivityLogScreen`, `PlannerActivityScreen`, `PlannerGroupScreen`, `WeekPlannerScreen`, etc.)
